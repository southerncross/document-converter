...
module Converter
  @@tchars = "萬與醜專業"
  @@schars = "万与丑专业"
  @@thash = {'萬'=>'万','與'=>'与','醜'=>'丑','專'=>'专','業'=>'业'}
  @@shash = {'万'=>'萬','与'=>'與','丑'=>'醜','专'=>'專','业'=>'業'}
  ...
  def s2t(str)
    return str.gsub(/([#{@@tchars}])/){|c|@@thash[c]}
  end

  def t2s(str)
    return str.gsub(/([#{@@schars}])/){|c|@@shash[c]}
  end

  def h2f(str)
    return str.gsub(/([#{@@hchars}])/){|c|@@hhash[c]}
  end

  def f2h(str)
    return str.gsub(/([#{@@fchars}])/){|c|@@fhash[c]}
  end
end
